const { getAllProduct, addproduct, editproduct, deleteProduct, getAllproducts,getOneProduct } = require('../model/productModel')
...
const Oneproduct = async (req, res) => {
    const id = req.params.id
    await getOneProduct(id).then((result) => {
        res.status(200).json(result)
    }).catch((err) => {
        res.status(500).json(err)
    })
...
module.exports = { getallpro, addpro, updateProduct, deletedProduct, getAll,Oneproduct }